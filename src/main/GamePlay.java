...
public class GamePlay {
	...
	public void game_play() throws Exception {
		...
		current_deck = new Deck().get_deck(); // 2. The game starts with a deck of 52 cards (a standard deck of shuffled playing cards).
		...
		while (true) {
			...
			Card top_discarded_card = discarded_pile_list.get(discarded_pile_list.size() - 1);  // Top discard card, with which the player will attempt to make a match with his cards.
			System.out.println("Top card discarded = " + discarded_pile_list.get(discarded_pile_list.size() - 1));
			...
			for (Card currentPlayerCard : players_list.get(current_player_turn).return_cards()) {
				...
				/* When it comes to the matching condition, 
				the player will attempt to match each of his cards 
				with the top card of the discard pile if the cards 
				have the same suits or amount of digits.*/

				if (currentPlayerCard.get_card_number() == top_discarded_card.get_card_number() || currentPlayerCard.get_current_suit() == top_discarded_card.get_current_suit()) {

					
					if (top_discarded_card.get_card_number() == 1 || top_discarded_card.get_card_number() == 11 || top_discarded_card.get_card_number() == 12 || top_discarded_card.get_card_number() == 13) {
						if (currentPlayerCard.get_card_number() == top_discarded_card.get_card_number())  // Even though it is available, the player cannot use the same action card, so he will skip.
							continue;
					}

					// 4. A player can only play a card if it matches either the suit or the rank of the top card on the discard pile.
					System.out.println("Cards matched for player " + players_list.get(current_player_turn).return_id()+ " Card and new Discard Deck top card = " + currentPlayerCard);

					
					
					if (number_cards_taken > 1) {   // To determine whether the current player must take more than one card or not, depending on the action card that the previous player may have played.
					...
					// If a player's card matches another player's, he or she discards the matching card from their hands and places it in the discard pile.
			...
			if (has_matched == false) {
				...
				// Depending on how many cards were drawn during the previous player's turn, the current player will take a certain number of cards from the draw pile.
				while (number_cards_taken-- > 0) {
					System.out.println("Drawing " + drawn_pile_list.get(drawn_pile_list.size() - 1) + " Card");
					players_list.get(current_player_turn).add_card(drawn_pile_list.get(drawn_pile_list.size() - 1));
					drawn_pile_list.remove(drawn_pile_list.size() - 1);
			...
			//5. A player has won a match if he has matched all of his cards and is the only one left. "Hurray!"
			if (has_matched == true && players_list.get(current_player_turn).return_cards().size() == 0) {
				System.out.println("Congratulations player " + players_list.get(current_player_turn).return_id() + " won the match !!!");			
				System.exit(0);
			}
			...
			// The following player's turn will be skipped if the player has already played the ACE card (Action card).
			if (has_matched == true && matched_number == 1) {
				current_player_turn += direction;
			}
			

			// The direction of the game's flow would be reversed if the player had played the KING card (an action card).
			if (has_matched == true && matched_number == 13) {
				direction *= -1;
			}
			
			
			// If the player has played the JACK card (Action card) then the next player have to take 4 cards from the draw pile.
			if (has_matched == true && matched_number == 11) {
				number_cards_taken = 4;
			}
			
			
			// A player must draw two cards from the draw pile if they have played the QUEEN card (Action card), in which case they must be taken by the following player.
